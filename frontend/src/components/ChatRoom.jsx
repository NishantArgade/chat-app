import React, { useState, useEffect, useRef } from "react";
import { useLocation } from "react-router-dom";
import Moment from "react-moment";
import { io } from "socket.io-client";

const ChatRoom = () => {
  const [data, setData] = useState({});
  const location = useLocation();
  const [msg, setMsg] = useState("");
  const [allMsg, setAllMsg] = useState([]);
  const [socket, setSocket] = useState();
  const msgDivRef = useRef(null);

  useEffect(() => {
    const socket = io("http://localhost:8000/");
    setSocket(socket);

    socket.on("connect", () => {
      // console.log(socket.id); // x8WIv7-mJelg7on_ALbx

      socket.emit("joinRoom", location.state.room);
    });
  }, [location]);

  useEffect(() => {
    socket?.on("getLatestMessage", (newMessage) => {
      setAllMsg([...allMsg, newMessage]);
    });
    msgDivRef.current.scrollIntoView({ behavior: "smooth" });
    setMsg("");
  }, [socket, allMsg, setAllMsg]);

  useEffect(() => {
    setData(location.state);
  }, [location]);

  const onSubmit = () => {
    if (msg) {
      const newMsg = {
        time: new Date().toLocaleString(),
        msg,
        name: data.name,
      };
      socket.emit("newMessage", { newMsg, room: data.room });
    }
  };
  const handleEnter = (e) => {
    if (e.keyCode === 13 && msg) onSubmit();
  };

  return (
    <div className="py-4 m-5 w-50 shadow bg-white text-dark border rounded container">
      <div className="text-center px-3 mb-4 text-capitalize">
        <h1 className="text-warning mb-4">{data?.room} Chat Room</h1>
      </div>
      <div
        className="bg-light border rounded p-3 mb-4"
        style={{ height: "450px", overflow: "scroll" }}
      >
        {allMsg.map((msgObj, index) => {
          return data.name === msgObj.name ? (
            <div className="row justify-content-end pl-5" key={index}>
              <div className="d-flex flex-column align-items-end m-2 shadow p-2 bg-info border round w-auto">
                <div>
                  <strong className="m-1">{msgObj.name}</strong>
                  <small className="text-muted m-1">
                    <Moment fromNow>{msgObj.time}</Moment>
                  </small>
                </div>
                <h4 className="m-1">{msgObj.msg}</h4>
              </div>
            </div>
          ) : (
            <div className="row justify-content-start " key={index}>
              <div className="d-flex flex-column  m-2 shadow p-2 bg-white border round w-auto">
                <div>
                  <strong className="m-1">{msgObj.name}</strong>
                  <small className="text-muted m-1">
                    <Moment fromNow>{msgObj.time}</Moment>
                  </small>
                </div>
                <h4 className="m-1">{msgObj.msg}</h4>
              </div>
            </div>
          );
        })}
        <div ref={msgDivRef}></div>
      </div>
      <div className="form-group d-flex">
        <input
          type="text"
          className="form-control bg-light"
          name="message"
          placeholder="Type your message"
          value={msg}
          onKeyDown={handleEnter}
          onChange={(e) => setMsg(e.target.value)}
        />
        <button
          type="button"
          className="btn btn-warning mx-2"
          onClick={onSubmit}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="16"
            height="16"
            fill="currentColor"
            class="bi bi-send"
            viewBox="0 0 16 16"
          >
            <path d="M15.854.146a.5.5 0 0 1 .11.54l-5.819 14.547a.75.75 0 0 1-1.329.124l-3.178-4.995L.643 7.184a.75.75 0 0 1 .124-1.33L15.314.037a.5.5 0 0 1 .54.11ZM6.636 10.07l2.761 4.338L14.13 2.576 6.636 10.07Zm6.787-8.201L1.591 6.602l4.339 2.76 7.494-7.493Z" />
          </svg>
        </button>
      </div>
    </div>
  );
};

export default ChatRoom;
